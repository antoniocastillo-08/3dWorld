...
@section('content')
    <div class="min-h-screen py-10 px-4 bg-gradient-to-br from-gray-100 to-gray-300">
        <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6">
            ...
            <form action="{{ route('models3d.update', $model->id) }}" method="POST" enctype="multipart/form-data"
                class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="file" id="file" name="file" accept=".stl"
                    ...
                    @if ($model->file)
                        <p class="mt-2 text-sm text-gray-600">Archivo actual: <a href="{{ asset('storage/' . $model->file) }}"
                                target="_blank" class="text-indigo-600 hover:underline">Descargar archivo</a></p>
                    @endif
                ...
                <!-- Imagen -->
                <div class="mb-4">
                    <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Imagen del
                        Modelo</label>
                    <input type="file" id="image" name="image" accept=".jpg,.jpeg,.png,.gif"
                        class="mt-1 block w-full text-sm text-gray-700 dark:text-white dark:bg-gray-700 dark:border-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    @if ($model->image)
                        <p class="mt-2 text-sm text-gray-600">Imagen actual: <a href="{{ asset('storage/' . $model->image) }}"
                                target="_blank" class="text-indigo-600 hover:underline">Ver imagen</a></p>
                    @endif
                ...
                <div class="flex justify-end space-x-4">
                    <a href="{{ route('models3d.show', $model->id) }}"
                        class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition">Cancelar</a>
                    <button type="submit"
                        class="bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-lg transition">Guardar
                        cambios</button>
                </div>
            </form>
        </div>
